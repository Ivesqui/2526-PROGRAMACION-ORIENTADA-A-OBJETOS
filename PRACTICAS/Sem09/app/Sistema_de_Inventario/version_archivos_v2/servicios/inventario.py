...
class Inventario:
    ...
    def agregar_producto(self, producto: Producto):
        if any(p.get_sku() == producto.get_sku() for p in self.__productos):
            return False
    ...
    def desactivar_producto(self, sku):
        for p in self.__productos:
            if p.get_sku() == sku and p.esta_activo():
                p._Producto__activo = False
                self.guardar_en_archivo()
                return True
        return False
        ...
        for p in self.__productos:
            if p.get_sku() == sku and p.esta_activo():
                ...
                if stock is not None:
                    p._Producto__stock_actual = stock

                if precio_compra is not None:
                    p._Producto__precio_compra = precio_compra

                if precio_venta is not None:
                    p._Producto__precio_venta = precio_venta
